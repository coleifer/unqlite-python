...
.. py:class:: UnQLite([filename=':mem:'[, flags=UNQLITE_OPEN_CREATE[, open_database=True]])
    ...
    .. py:method:: get(key[, default=None])
        ...
        Fetch the value at the given ``key``, returning ``default`` if the
        key does not exist. Behaves like :py:meth:`dict.get`.

        :param str key: The identifier of the value to fetch.
        :param default: Value to return if the key does not exist.
        :returns: The data stored at the given key, or ``default``.
    ...
    .. py:method:: match_prefix(prefix)

        Iterate over all key/value pairs whose key begins with the given
        ``prefix``, yielding 2-tuples of key and value.

        .. code-block:: python

            for key, value in db.match_prefix('user.'):
                print(key, '=>', value)

        .. warning::
            The default UnQLite storage engines are hash-based and do not
            store keys in sorted order, so this method performs an O(n) scan
            of the database.
    ...
    .. py:method:: set_max_page_cache(npages)

        Suggest the maximum number of raw pages UnQLite should cache in
        memory. This is a hint; UnQLite is free to ignore it.

        :param int npages: Maximum number of pages to cache.

    .. py:method:: kv_engine()

        :returns: The name of the underlying key/value storage engine, e.g.
            ``'hash'`` for file-based databases or ``'mem'`` for in-memory
            databases.
...
.. py:class:: VM(unqlite, code)
    ...
    .. py:method:: output()
        ...
        Retrieve the output generated by the executed Jx9 script, e.g. by
        calls to ``print``.

        .. code-block:: python

            with db.vm('print "hello, "; print $name;') as vm:
                vm['name'] = 'charlie'
                vm.execute()
                vm.output()  # 'hello, charlie'
